...
async def loginHandler(c: Client, m: Message):
    ...
    if user.user_id == int(Config.OWNER):
        user.allowed = True
    if user.allowed:
        await m.reply_text(text=f"**Do not Spam**\n  ⚡ You can use me!!", quote=True)
    else:
        try:
    ...
    if user.merge_mode == 4:
        ...
        if rmess.video or rmess.document:
            ...
            if file_name is None:
                await m.reply("File name not found; goto @doraemonbots")
                return
...
🔰 ᴏᴡɴᴇʀ ᴄᴀɴ ʙʀᴏᴀᴅᴄᴀsᴛ ᴍᴇssᴀɢᴇ ᴛᴏ ᴀʟʟ ᴜsᴇʀs
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("👨‍💻Developer👨‍💻", url="https://graph.org/Doraemon-Bots-05-01")],
                [
                    InlineKeyboardButton(
                        "🏘Want Bot Like This ?🏘", url="https://graph.org/Doraemon-Bots-05-01"
                    ),
                    InlineKeyboardButton(
